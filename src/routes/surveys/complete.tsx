...
import {
  Shield, Download, CheckCircle2, XCircle,
  AlertTriangle, GraduationCap, ArrowRight, Phone,
} from "lucide-react";
...
function AnimatedCheckmark() {
  return (
    <svg viewBox="0 0 52 52" style={{ width: 36, height: 36 }} aria-hidden="true">
      <style>{`
        @keyframes ts-circ { 0%{stroke-dashoffset:150.8} to{stroke-dashoffset:0} }
        @keyframes ts-tick  { 0%{stroke-dashoffset:30}   to{stroke-dashoffset:0} }
      `}</style>
      <circle cx="26" cy="26" r="24" fill="none" stroke={TEAL} strokeWidth="2.5"
        strokeDasharray="150.8" strokeDashoffset="150.8"
        style={{ animation: "ts-circ 0.5s ease forwards" }} />
      <path d="M14 27 l8 8 l16-16" fill="none" stroke={TEAL} strokeWidth="2.5"
        strokeLinecap="round" strokeLinejoin="round" strokeDasharray="30" strokeDashoffset="30"
        style={{ animation: "ts-tick 0.3s 0.5s ease forwards" }} />
    </svg>
  );
}
...
function CompletePage() {
  ...
  const [domainScores, setDomainScores] = useState<number[]>(B_DOMAINS.map(() => 0));
  const [totalScore, setTotalScore] = useState(0);

  useEffect(() => {
    try {
      const b = JSON.parse(localStorage.getItem("ts_survey_sectionB") || "{}") as Record<string, string>;
      const c = JSON.parse(localStorage.getItem("ts_survey_sectionC") || "{}") as Record<string, string>;
      const d = JSON.parse(localStorage.getItem("ts_screening_demographics") || "{}") as Demographics;
      setSectionB(b);
      setSectionC(c);
      setDemographics(d);
      ...
      console.error("[report] localStorage read error:", e);
    } finally {
      setReady(true);
    }
  }, []);
...
<title>Cybersecurity Report — ${demographics.businessName ?? "Your Business"}</title>
<style>
${styles}
...
body { font-family:'DM Sans',sans-serif; background:white; color:#1a1a2e; margin:0; padding:0; }
  ...
  const strengths = Object.keys(B_RECS)
    ...
    .map((k) => B_RECS[k].gap.replace("No ", "Has ").replace("Not ", ""));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="no-print sticky top-0 z-10 border-b bg-white px-6 py-3 flex items-center justify-between shadow-sm">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">Your cybersecurity report is below</div>
          </div>
        </div>
        <button
      ...
      {/* Report — always rendered; state updates fill in data after mount */}
      <div ref={printRef} style={{ fontFamily: "'DM Sans',sans-serif", color: "#1a1a2e", background: "white" }}>
        ...
        {gaps.length > 0 && (
          <Section title="Where Action Is Needed">
            <p style={{ fontSize: 14, color: "#64748b", marginBottom: 24, lineHeight: 1.6 }}>
              Each gap includes a plain-English explanation, a practical recommendation, and a Tetrasec service that can help.
            </p>
            {B_DOMAINS.map((dom) => {
              ...
              return (
                <div key={dom.key} style={{ marginBottom: 32 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, paddingBottom: 10, borderBottom: `2px solid ${NAVY}18` }}>
                    ...
                    <span style={{ fontSize: 12, color: "#94a3b8", marginLeft: "auto" }}>{domGaps.length} gap{domGaps.length > 1 ? "s" : ""}</span>
                  </div>
                    ...
                    {domGaps.map(({ key, rec }) => (
                      <div key={key} style={{ border: "1px solid #fee2e2", borderLeft: "4px solid #ef4444", borderRadius: 10, padding: 18, background: "#fff5f5" }}>
                        ...
                        <div style={{ marginBottom: 10 }}>
                          <span style={{ fontSize: 12, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em" }}>What this means</span>
          ...
          <Section title="Your Website Security" bg="#f8fafc">
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#64748b" }}>Scanned: {demographics.websiteUrl}</div>
              </div>
            </div>
        ...
        <Section title="How Tetrasec Solutions Can Help" bg={`${NAVY}08`}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 14, marginBottom: 28 }}>
            {[
              ...
              { title: "Security Awareness Training", desc: "Engaging, Kenya-specific training that turns your staff into your first line of defence." },
              ...
              { title: "Virtual CISO", desc: "Fractional security leadership — the expertise of a CISO at an SME-friendly cost." },
            ].map((s) => (
          ...
          <div style={{ background: NAVY, borderRadius: 12, padding: "24px 28px", display: "flex", flexWrap: "wrap", gap: 24, alignItems: "center" }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.65)" }}>Tell us your top three gaps and we'll outline the most cost-effective path forward.</div>
            </div>
            ...
            <div style={{ background: NAVY, padding: "24px 28px", display: "flex", alignItems: "flex-start", gap: 18 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.65)", marginTop: 6, maxWidth: 480 }}>
                  A 6-week, Kenya-focused course for SME owners and managers — no coding required.
                </div>
              </div>
            </div>
              ...
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                {["Spot your most profitable customers", "Forecast cash flow in Google Sheets", "Understand Kenya DPA 2019"].map((item) => (
                  <div key={item} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#374151" }}>
                    <CheckCircle2 style={{ color: "#22c55e", width: 14, height: 14 }} />{item}
                  </div>
                ))}
              </div>
